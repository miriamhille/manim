...
class Shapes(Scene):
    ...
    def construct(self):
        ...
        triangle=Polygon(np.array([0,0,3]),np.array([1,1,0]),np.array([1,-1,0]))
...
if __name__ == "__main__xxxxx":
    ...
    for item in module_info.values():
        if item.module==module_name:
            print(item.name)
            os.system("python3.7 -m manim manim_tutorial_P37.py %s -l" % item.name)  #Does not play files


if __name__ == "__main__":
    module_name = os.path.basename(__file__)
    command = "python3.7 -m manim  -p  --leave_progress_bars " + module_name
    os.system(command)
